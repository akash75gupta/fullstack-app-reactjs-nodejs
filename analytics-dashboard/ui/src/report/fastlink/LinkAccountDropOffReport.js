...
export default class LinkedAccountDropOffReport extends Component {
  static propTypes = {
  ...
  //console.log("Constructing Linked Account Drop Off Report.");
    ...
    this.state = {
      header:{
        ...
        subtitle: reportSpecification.report.subtitle,
  ...
  //console.log("Rendering <LinkedAccountDropOffReport> for fetchedReport: "+JSON.stringify(fetchedReport));
    let reportHeaderSupplymentaryData = null;
    let returnJsx = null;
    ...
    if(underscore.isEqual(fetchedReport.status, Constants.REPORT_ERROR_STATUS)){
      ...
      returnJsx =  <ChartLoader />;
        ...
        if(underscore.isEqual(fetchedReport.data, {})){
          ...
          reportHeaderSupplymentaryData = this.getTotalPercentageDrop(fetchedReport.data);
    ...
    return (
      <Row type='flex' justify="center">
        <Col span={24}>
          <ReportHeader data={this.state.header} enabled={headerEnabled} supplymentaryData={reportHeaderSupplymentaryData}/>
          {returnJsx}
        </Col>
      </Row>
    );
