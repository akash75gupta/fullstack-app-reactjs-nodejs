...
export default class CapturedSearchTextReport extends Component {
  static propTypes = {
  ...
  //console.log("Constructing Linked Account Drop Off Report.");
    super(props);
    let reportSpecification = ProjectUtil.getReportSpecification(Config.DEFAULT_PROJECT, Config.MYAPP_REPORT_4);
    this.state = {
      header:{
  ...
  render() {
    let fetchedReport = ReportUtil.findReportById(Config.MYAPP_REPORT_4, this.props.context.reports);
  //console.log("Rendering <CapturedSearchTextReport> for fetchedReport: "+JSON.stringify(fetchedReport));
